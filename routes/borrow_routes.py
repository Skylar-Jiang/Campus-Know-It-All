...
def register_borrow_routes(app):
    ...
    def borrow_manage():
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT order_id, activity_id, applicant_user_id, apply_time,
        ...
        return render_template("borrow_manage.html", orders=orders, resources=resources, activities=activities)
    ...
    def borrow_detail_add():
        ...
        if not order_id.isdigit() or not resource_id.isdigit() or not borrow_qty.isdigit():
            flash("明细参数非法。", "error")
            return redirect(url_for("borrow_manage"))
        ...
        with get_db_connection() as conn:
            try:
                with conn.cursor() as cur:
                    cur.execute("SELECT applicant_user_id FROM borrow_order WHERE order_id=%s", (order_id,))
                    order = cur.fetchone()
                    if not order:
                        flash("借用单不存在。", "error")
                        return redirect(url_for("borrow_manage"))
