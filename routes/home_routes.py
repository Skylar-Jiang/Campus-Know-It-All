    ...
    def home_dashboard():
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    SELECT competition_id, title, category, start_time
                    FROM competition
                ...
                latest_competitions = cur.fetchall()
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                if user['role'] == 'club' and user.get('club_id'):
                    ...
                    cur.execute(
                        """
                        SELECT points, violation_count, credit_score
                        FROM student
                        WHERE student_id = %s
                        """,
                        (user['student_id'],),
                    )
                    data['student_profile'] = cur.fetchone()
