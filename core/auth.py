...
def can_manage_competition(user, competition):
    if user["role"] == "admin":
        return True
    if user["role"] == "club" and user.get("club_id") == competition["club_id"]:
        return True
    return False
...
def init_auth_hooks(app):
    ...
    def context_data():
        return {
            ...
            "activity_categories": current_app.config["ACTIVITY_CATEGORIES"],
            "competition_categories": current_app.config["COMPETITION_CATEGORIES"],
            "competition_status": current_app.config["COMPETITION_STATUS"],
        }
